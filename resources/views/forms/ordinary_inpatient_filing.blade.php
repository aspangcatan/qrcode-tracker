<div class="container">
    ...
    <div class="certificate-text">
        ...
        <div>
            ...
            was examined and treated / confined in<br/>
        </div>
        <div>
            this hospital on/from
            <div class="small">
                @if(isset($certificates) && $certificates)
                    <input type="date" id="date_examined"
                           value="{{ \Illuminate\Support\Carbon::parse($certificates->date_examined)->format('Y-m-d') }}"
                           onchange="document.getElementById('date_discharged').value = this.value"/>
                @else
                    <input type="date" id="date_examined" onchange="document.getElementById('date_discharged').value = this.value"/>
                @endif
            </div>
            to
            <div class="small" style="border-bottom: 1px solid black; display: inline-block; text-align: center; width: 180px;">
                PRESENT
            </div>
            @if(isset($certificates) && $certificates)
                <input type="hidden" id="date_discharged" value="{{ \Illuminate\Support\Carbon::parse($certificates->date_examined)->format('Y-m-d') }}"/>
            @else
                <input type="hidden" id="date_discharged"/>
            @endif
